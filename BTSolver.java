...
public class BTSolver
{
	...
	{
		Set<Variable> modified_vars = new HashSet<>();
		...
		for (Constraint c: network.getModifiedConstraints()) {
			for(Variable v : c.vars) {
				if( v.isAssigned() && modified_vars.contains(v)) 
				{
					for(Variable var : c.vars) 
					{
						...
						Integer val = v.getAssignment();

						if (!var.getValues().contains(val))
							continue;

						if (var.size() == 1 && var.getAssignment().equals(val))
							return false;
	...
	{
		ArrayList<Integer> values = v.getDomain().getValues();
		List<Variable> neis = network.getNeighborsOfVariable(v);
		Map<Integer, Integer> hash = new HashMap<>();

		for (Integer value : values) {
			hash.put(value, 0);
			for (Variable nei : neis) {
				if (nei.getDomain().getValues().contains(value))
